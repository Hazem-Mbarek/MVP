...
export const TASK_RULES = {
  incoterms: {
    ...
    rules: [
      "TOOLS:",
      "  - compare_incoterms(codes: string[]) — compares Incoterm attributes from YAML source",
      "  - search_knowledge(query: string, source_filter: 'incoterms', top_k: 5) — searches Incoterm definitions",
      ...
      "CRITICAL: You MUST execute tools before answering. Do not answer from memory.",
      "Never use search_knowledge to answer attribute/obligation questions - those are YAML-only questions",
      "Cite every fact: [record_id, source] or [filename]",
      "Always execute tools with well-formed inputs",
    ...
    constraints: [
      ...
      "Tools MUST be executed for every task",
  ...
  cmr: {
    ...
    tools: ["search_knowledge"],
    rules: [
      "TOOL: search_knowledge(query: string, source_filter: 'cmr', top_k: 5) — searches CMR documents for legal provisions",
      ...
      "CRITICAL: You MUST execute search_knowledge before answering. Do not answer from memory.",
      "Always execute the tool with a well-formed query",
    ...
    constraints: [
      ...
      "No answers from model memory - MUST use tool",
      "Tool must be executed for every task",
    ],
  },
  ...
  faq: {
    ...
    tools: ["search_knowledge"],
    rules: [
      "TOOL: search_knowledge(query: string, source_filter: 'faq', top_k: 5) — searches FAQ for policies and practices",
